...
async def responser(update: Update, context: ContextTypes.DEFAULT_TYPE, numbers=(), html_bl=False):
    ...
    numbers.sort()
    message = await update.message.reply_text("⏳ يتم جلب المعلومات من الموقع ... انتظر ")
    try:
        async with aiohttp.ClientSession() as session:
...
async def one_req(number, session: aiohttp.ClientSession, recurse=0) -> dict[bytes, int]:
    ...
    url = "https://exam.albaath-univ.edu.sy/exam-civil/re.php"
...
def init_config_file():
    token = input("6422430655:AAEIjACABKcHovv6-IyGmPr9dYVzP9AyYrU):\n")
